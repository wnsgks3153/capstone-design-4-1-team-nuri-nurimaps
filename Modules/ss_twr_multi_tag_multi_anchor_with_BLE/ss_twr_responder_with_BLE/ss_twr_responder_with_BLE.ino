...
/* 송신 및 수신 장치 주소 및 PAN ID 정의
  ...
  기본 예제 코드는 1:1 통신을 기준으로 Initiator의 주소와, responder의 주소를 정적으로 설정해두었으나
  다수의 앵커 다수의 태그를 사용하도록 설계하면서 태그의 주소값이 저장되는 DEST_ADDR를 태그로부터 동적으로 메세지를 받아
  추출된 태그의 주소를 DEST_ADDR 변수에 저장되도록 변경 */

#define DEST_PAN_ID 0x4321          /* PAN ID (네트워크 식별자) */
#define SRC_ADDR 0x0201500100300030 /* 이니시에이터(발신기) 주소 */
...
static dwt_config_t config = {
  5,                /* 채널 번호 */
  DWT_PLEN_128,     /* 프리앰블 길이 (TX 전송 시 사용) */
  DWT_PAC8,         /* 프리앰블 획득 청크 크기 (RX 수신 시 사용) */
  9,                /* 송신 프리앰블 코드 */
  9,                /* 수신 프리앰블 코드 */
  1,                /* SFD 모드 (0: 표준 8 심볼, 1: 비표준 8 심볼, 2: 비표준 16 심볼, 3: 4z 8 심볼) */
  DWT_BR_6M8,       /* 데이터 속도 (6.8 Mbps) */
  DWT_PHRMODE_STD,  /* PHY 헤더 모드 */
  DWT_PHRRATE_STD,  /* PHY 헤더 속도 */
  (129 + 8 - 8),    /* SFD 타임아웃 (프리앰블 길이 + 1 + SFD 길이 - PAC 크기, RX 수신 시 사용) */
  DWT_STS_MODE_OFF, /* STS 비활성화 */
  DWT_STS_LEN_64,   /* STS 길이 (가능한 값은 `dwt_sts_lengths_e` 열거형 참조) */
...
/* 거리 측정 과정에서 사용되는 MAC 페이로드 데이터. 아래의 주석 3을 참고 */
...
void communicateWithTag() {
  ...
  if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) {
    ...
    // 프레임 수신 완료 후 호출되는 부분에서:
    dwt_readrxdata(rx_buffer, frame_len, 0);

    /* 수신된 데이터에서 태그 주소 추출 (PAN ID 이후 8바이트가 Dest, 그 다음 8바이트가 Src)
        13으로 설정된 이유 = 2(FrameCtrl) + 1(Seq) + 2(PAN) + 8(Dest_addr) + 8(Src_addr) + (payload, etc...)
        즉, 인덱스 13부터 태그가 보낸 메세지에서 태그의 주소가 담겨 있음을 알 수 있음
    */
    memcpy(&DEST_ADDR, &rx_buffer[13], sizeof(uint64_t));

    /* 메세지를 보낸 태그의 주소를 확인하기 위해 디버그를 수행하는 부분
       char buf[64];
       sprintf(buf, "수신한 태그 주소: 0x%llX", DEST_ADDR);
       Serial.println(buf); */
    ...
    /* 오류 발생 시 처리 
       ...
       에러를 무한 출력하며, 통신 불가능 상태가 되는 while문 제거 및 본인(이 코드를 사용하는 앵커)을 향한 메세지가 아니면 
    ...
    if (status != AES_RES_OK) {
      ...
      delay(100);
      return;
    }  // 현재 이 부분은 디버그용으로 사용하기위해 시리얼 모니터로 출력하게 되어있음 실제 사용을 할 계획인 경우 출력 부분을 삭제하거나 주석 처리 할 것.
